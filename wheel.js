// segments + GIFs (with numbering)
const segments = [
  { label: "1: ₹1", value: 1, type: "win" },
  { label: "2: Double", value: null, type: "double" },
  { label: "3: ₹5", value: 5, type: "win" },
  { label: "4: Try Again", value: 0, type: "lose" },
  { label: "5: ₹3", value: 3, type: "win" },
  { label: "6: Try Again", value: 0, type: "lose" },
  { label: "7: ₹10", value: 10, type: "win" },
  { label: "8: ₹2", value: 2, type: "win" }
];
const gifMap = {
...
// wheel build with numbering visually
function buildWheel(){
  ...
  segments.forEach((seg,i)=>{
    ...
    // label with numbering and main text
    div.innerHTML = `
      <div style="transform:skewY(60deg); display:flex; flex-direction:column; gap:4px;">
        <div style="font-size:12px; font-weight:700;">${seg.label}</div>
        <div style="font-size:10px; opacity:.9;">#${i+1}</div>
      </div>`;
...
// modified spin logic requiring watch click after each result
let spinning=false;
let needsContinueClick=false;

spinBtn.disabled=false;
watchBtn.style.display="none";

spinBtn.addEventListener("click", ()=>{
  if(spinning || needsContinueClick) return;
  spinning=true;
  spinBtn.disabled=true;
  watchBtn.style.display="none";

  const { segment,index } = (function(){ const idx=Math.floor(Math.random()*segments.length); return { segment: segments[idx], index: idx }; })();
  ...
  setTimeout(()=>{
    ...
    spinning=false;
    // require user to click continue
    needsContinueClick=true;
    watchBtn.textContent="▶ Watch & Continue to Proceed";
...
watchBtn.addEventListener("click", ()=>{
  if(!needsContinueClick) return;
  const link= advanceAdIndex();
  if(link) window.open(link,"_blank");
  needsContinueClick=false;
  spinBtn.disabled=false;
  watchBtn.style.display="none";
});
...
document.getElementById("goSupport")?.addEventListener("click",(e)=>{ e.preventDefault(); openTelegramWithText(); });
document.getElementById("supportInput")?.addEventListener("keypress",(e)=>{ if(e.key==="Enter") openTelegramWithText(); });
